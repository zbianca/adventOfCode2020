...
Part one: Each line gives the password policy and then the password. The password policy indicates the lowest and highest number of times a given letter must appear for the password to be valid. For example, 1-3 a means that the password must contain a at least 1 time and at most 3 times.
Part two: Each line actually describes two positions in the password, where 1 means the first character, 2 means the second character, and so on (no concept of "index zero"!). Exactly one of these positions must contain the given letter.
...
function finder(passwords, validator) {
  let count = 0;
  const list = passwords.trim().split("\n");
  list.forEach(line => {
    const [_, min, max, letter, pass] = line.match(/(\d+)-(\d+) (\w): (\w+)/);
    if (validator(min, max, letter, pass)) {
      count += 1;
    }
  });
  return count;
...
function isXor(i, j, letter, pass) {
  // to 'index zero' format
  i -= 1;
  j -= 1;
  return (pass[i] === letter) !== (pass[j] === letter);
}

// console.log(finder(smallInput, isValid)); // -> 2
// console.log(finder(input, isValid)); // -> 416
console.log(finder(input, isXor)); // -> 688