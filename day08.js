...
function getAccumulator(input) {
  const pdata = input.trim().split('\n');
  const visited = new Array(pdata.length).fill(false);
  let acc = 0;
  let i = 0
  while (!visited[i] && i >= 0) {
    visited[i] = true;
    let [cmd, num] = pdata[i].split(' ');
    num = parseInt(num);
    if (cmd === 'nop') {
      i++;
    } else if (cmd === 'acc') {
...
function fixBoot(input) {
  // exchange jmp and nops
}

// console.log(getAccumulator(smallInput)); // -> 5
// console.log(getAccumulator(input)); // -> 1134
// console.log(fixBoot(smallInput)); // -> 8
console.log(fixBoot(input)); // -> !2165