...
function validateReq(pass) {
  const req = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];
  let found;
  let i = 0;
  while (found !== -1 && i < req.length) {
    found = pass.indexOf(req[i] + ':');
    i++;
  }
  return found !== -1;
}
...
function validate(input) {
  ...
  list.forEach(pass => {
    if (validateReq(pass)) {
      ...
      if (validateFields(format(pass))) count++;
    }
  });
  return count;
}
...
function format(string) {
  const register = {};
  string.split(/\s/).forEach(item => {
    let kv = item.split(':');
    if (kv[0] === 'byr' || kv[0] === 'iyr' || kv[0] === 'eyr') {
      register[kv[0]] = parseInt(kv[1]);
    } else register[kv[0]] = kv[1];
  });
  return register;
}

function validateFields(pass) {
  return (
    (pass.byr >= 1920 && pass.byr <= 2002) &&
    (pass.iyr >= 2010 && pass.iyr <= 2020) &&
    (pass.eyr >= 2020 && pass.eyr <= 2030) &&
    validateHeight(pass.hgt) &&
    validateEcl(pass.ecl) &&
    /^\d{9}$/.test(pass.pid) &&
...
function validateHeight(height) {
  ...
  let [_, value, unit] = height.match(/(\d+)(cm|in)/);
  value = parseInt(value);
  return ((unit === 'cm' && value >= 150 && value <= 193) || (unit === 'in' && value >= 59 && value <= 76))
}

// console.log(countValid(smallInput)); // -> 2
// console.log(countValid(input)); // -> 182
console.log(validate(input)); // -> 109