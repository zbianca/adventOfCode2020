...
function processData(input) {
  ...
  const rules = {};
  lines.forEach(line => {
    let [_, name, content] = line.match(/(\w+ \w+) bags contain (.*)/);
    const contents = new Map();
    Array.from(content.matchAll(/(\d+) (\w+ \w+)/g)).forEach(el => {
      contents.set(el[2], el[1]);
    });
    rules[name] = contents;
  });
  return rules;
...
const pdata = processData(input);

function bagModelCounter(model) {
  const result = [];
  for (const [key, value] of Object.entries(pdata)) {
    if (value.has(model)) result.push(key);
  }

  let i = 0;
  do {
    for (const [key, value] of Object.entries(pdata)) {
      if (value.has(result[i]) && !result.includes(key)) result.push(key);
    }
    i++;
...
function bagCounter(model) {
  let children = pdata[model];
  return addAll(children, 1);
}

function addAll(bags, mult) {
  let total = 0;

  if (bags && bags.size > 0) {
    const iterator = bags.keys();
    for (let item of iterator) {
      const count = bags.get(item);
      total += count * mult + addAll(pdata[item], mult * count)
    }
  }
  return total;
}

console.log(bagModelCounter('shiny gold')); // -> 370
console.log(bagCounter('shiny gold')); // -> 29547
