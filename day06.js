...
function groupSome(group) {
  const groupSet = new Set;

  group.forEach(person => {
    for (let i = 0; i < person.length; i++) {
      groupSet.add(person[i]);
    }
  });
...
// Another solution could be writing a polyfill method for computing the intersection of sets.
function groupEvery(group) {
  const answers = group.reduce((acc, cur) => {
    let result = '';
    for (let i = 0; i < acc.length; i++) {
      if (cur.includes(acc[i])) result += acc[i];
    }
    return result;
  });
...
function addGroups(input, fun) {
  let total = 0;
  const list = input.trim().split('\n\n');
  list.forEach(group => {
    total += fun(group.split('\n'));
  });
  return total;
}

// console.log(addGroups(smallInput, groupSome)) // -> 11
// console.log(addGroups(input, groupSome)) // -> 6521
// console.log(addGroups(smallInput, groupEvery)) // -> 6
console.log(addGroups(input, groupEvery)) // -> 3305