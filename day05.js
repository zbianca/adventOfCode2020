...
function seatId(string) {
  const binary = [
    ...
    ['L', '0'],
    ['R', '1']
  ].reduce((acc, cur) => acc.replaceAll(cur[0], cur[1]), string);
...
function seater(input) {
  const seats = new Array(1024).fill(0);

  input.forEach(string => {
    seats[seatId(string)] = 1;
  });

  const min = seats.indexOf(1);
  return seats.indexOf(0, min);

}

// console.log(seatId('FFFBBBFRRR')); // -> 119
// console.log(findHighest(list)); // -> 801
console.log(seater(list)); // -> 597